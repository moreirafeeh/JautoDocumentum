...
public class UtilsDocumentum extends conexao_documentum {
	...
	// -------------------------EXPORTAR------------------------
	public void exportVirtualDocument(String tipo_busca, String Obj_Busca)
			throws Exception {
		...
		if (tipo_busca == "id") {
			...
			IDfSysObject sysObject2 = (IDfSysObject) sessDctm
					.getObject(sysObjId);
		...
		try {
			if (sysObject.isVirtualDocument()) {
				...
				IDfVirtualDocument Vdoc = sysObject.asVirtualDocument(
						"CURRENT", false);

				IDfExportNode expNode = (IDfExportNode) expOperation
						.add(sysObject);
	...
	public void EXPORT_Normal_doc() {
		...
		try {
			...
			sysObj = (IDfSysObject) getSessDctm()
					.getObjectByPath(
							"/Sinistros Autos/S100020106000539/DPA01/DPA01_Abertura (060999.pdf)");

			// Creating the Export operation object
			IDfExportOperation operation = new DfClientX().getExportOperation();

			// Setting the Destination directory to which the Documents will be
			// exported to
			operation.setDestinationDirectory("C:\\temp\\documentum");

			// Adding the object(to be exported)to the Export Operation
			// More than one document can be added, just repeat the below step
			// for it
	...
	public void ImportarDocumentum() {
		...
		try {
			...
			sysObj
					.setObjectName("NFO_000000000000000_00000000000000000_0000000_00000000000000.PDF");
			sysObj.setContentType("pdf");
			sysObj
					.setFile("C://Users//lvidotti//Desktop//mapfre//Ambinte//Export-documeto-documentum//NFO_000000000000000_00000000000000000_0000000_00000000000000.PDF");

			sysObj.link("/Sinistros Autos/Não Indexados");
			sysObj.save();
	...
	public void getAllDocbases() throws Exception {
		...
		for (int i = 0; i < docbaseMap.getDocbaseCount(); i++) {
			...
			System.out
					.println("Docbase Name : " + docbaseMap.getDocbaseName(i));
	...
	// ---------------------------------------------------------
	// ----------------Criar Pasta (necessario passar cabinet onde a pasta
	// ficará)------------------------------
	...
	// EXEMPLOS
	// createDocument("Felipe_teste_func", "crtext",
	// "C:\\Documentum\\export\\testandoTwitch.txt"
	// ,"/Felipe Twitch/felipinho3");
	// -------
	...
	// ---------------------------------------------------------
	// updateAttributes("/Felipe Twitch/felipinho3/Felipe_teste_func",
	// "object_name", "meu_doc_alterado")
	// ------------------Atualizar Atributos--------------------MELHORAR
	public void updateAttributes(String path_conteudo_documentum,
			String nome_atributo, String atributo_alterado) throws Exception {

		IDfSysObject sysObject = null;

		sysObject = (IDfSysObject) getSessDctm().getObjectByPath(
				path_conteudo_documentum);
	...
	public void createVirtualDocument() throws Exception {
		...
		IDfSysObject pSys = (IDfSysObject) getSessDctm().getObjectByPath(
				"/Training Cabinet XXX/Folder Level 2/log4j.properties");

		IDfSysObject cSys = (IDfSysObject) getSessDctm().getObjectByPath(
				"/Training Cabinet XXX/Folder Level 1/trace.log");
	...
	public void viewVirtualDocument() throws Exception {
		...
		IDfSysObject pSys = (IDfSysObject) getSessDctm().getObjectByPath(
				"/Felipe Twitch/felipinho3/alterei_denovo");
	...
	// -----------------
	// antes de usar esse metodos é necessario utilizar o
	// "consultarquery() para puxar o arquivos";
	public void assignACL(ArrayList<String> arquivos) throws Exception {
		...
		for (int i = 0; i < arquivos.size(); i++) {
			...
			IDfSysObject mDocs = (IDfSysObject) getSessDctm()
			...
			IDfACL dfACL = (IDfACL) getSessDctm().getObjectByQualification(
					"dm_acl where object_name='LCS_felipeTW_not'");
	...
	public void startWorkflow() throws Exception {
		...
		IDfSysObject sysObj = (IDfSysObject) getSessDctm()
				.getObjectByQualification(
						"dm_document where object_name='DFCtest'");

		// to get the Workflow Template
		IDfProcess process = (IDfProcess) getSessDctm()
				.getObjectByQualification(
						"dm_process where object_name like '%INDIA_TIMES%'");

		// to create a Workflo Builder to start a workflow
		IDfWorkflowBuilder builder = getSessDctm().newWorkflowBuilder(
				process.getObjectId());
		...
		IDfActivity activity = (IDfActivity) getSessDctm()
	...
	// -------------------CONSULTAS DQL array de não indexados--------------------
	public ArrayList<String> SrcClear(String queryString) throws Exception {
		...
		while (coll.next()) {
			...
			for (String param : params) {
				...
				param.replaceAll(".", "");
				param.replaceAll("PDF", "");
				param.replaceAll("pdf", "");

				 // add se tiver placa
				if (param.length() == 7
						&& param.substring(0, 3).matches("[A-Z]*")) {
					arquivo.add(typeObject.getString("resultado_query"));
				}
				 //--------------------------------------------------------
				else {
					// add se tiver somente numero falta especificar  ===
					...
					// --------------------------------------------------------
						...
						ConsultarQueryUPDATE(Querys.UPDATE_LINK(
								"/Lucas Vidotti/ParametrosIncorretos",
								objectNameFile));
						ConsultarQueryUPDATE(Querys.UPDATE_UNLINK(
								"/Sinistros Autos/Não Indexados",
								objectNameFile));
